...
function wptouch_has_pro_module( $module_name ) {
	return file_exists( WPTOUCH_DIR . '/' . $module_name . '.php' );
}
...
function wptouch_can_show_license_menu() {
	...
	if ( !wptouch_is_multisite_enabled() ) {
		...
		if ( function_exists( 'get_site_option' ) ) {
			$should_show_license_menu = wptouch_is_multisite_primary() && !get_site_option( WPTOUCH_MULTISITE_LICENSED, 0, false );
		} 
	}
...
function wptouch_should_show_license_nag() {
	$should_show_nag = true;
	
	if ( !wptouch_is_multisite_enabled() ) {
		$settings = wptouch_get_settings();
		$should_show_nag = ( !$settings->license_accepted );
	} else {
		if ( function_exists( 'get_site_option' ) ) {
			$should_show_nag = !get_site_option( WPTOUCH_MULTISITE_LICENSED, 0, false );
		} 
	}
	
	return $should_show_nag;
}

function wptouch_should_show_activation_nag() {
	if ( wptouch_is_multisite_enabled() ) {
		return !get_site_option( WPTOUCH_MULTISITE_LICENSED, 0, false ) && wptouch_is_multisite_primary();
	} else return wptouch_should_show_license_nag();
}
...
function wptouch_is_multisite_licensed() {
	$licensed = 0;
	if ( wptouch_is_multisite_enabled() ) {
		if ( function_exists( 'get_site_option' ) ) {
			$licensed = get_site_option( WPTOUCH_MULTISITE_LICENSED, 0, false );
		}
	}
