<?php
add_action( 'wptouch_admin_page_render_wptouch-admin-theme-options', 'foundation_social_links_settings' );
...
function foundation_social_show_one_link( $href, $social_service, $friendly ) {
	$link = '<a href="' . $href . '" class="social-footer-badges no-ajax icon-' . $social_service . '" role="button" title="' . $friendly . '" target="_blank"></a>';
...
function foundation_social_links(){
	...
	if ( $settings->social_facebook_url ) {
		foundation_social_show_one_link( $settings->social_facebook_url, 'facebook', 'Facebook' );
	} 
	...
	if ( $settings->social_rss_url ) {		
		foundation_social_show_one_link( $settings->social_rss_url, 'rss', 'RSS' );
	}
